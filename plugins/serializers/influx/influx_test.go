...
func Yolo(v telegraf.Metric, err error) telegraf.Metric {
	if err != nil {
		panic(err)
	}
	return v
...
func TestSerializer(t *testing.T) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Sort the fields for reproducible results
			sort.Slice(tt.input.FieldList(), func(i, j int) bool {
				return tt.input.FieldList()[i].Key < tt.input.FieldList()[j].Key
			})

			serializer := NewSerializer(tt.maxBytes)
